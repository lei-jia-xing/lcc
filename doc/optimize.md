...
## 12. 优化 Pass：CFG 稀疏条件常量传播（SCCP）

### 12.1 实现概述

在基本块/CFG 级别执行稀疏条件常量传播与不可达块删除：

- 维护临时寄存器的三值格（Unknown/Constant/Overdefined）。
- 按可达块工作队列传播，遇到常量条件的 `IF` 仅标记真/假一侧后继可达。
- 根据可达信息清理 PHI 参数，单入边 PHI 退化为 ASSIGN；无入边 PHI → NOP。
- 常量折叠后把 `IF` 直接改为 `GOTO` 或 `NOP`，删除不可达后继并裁剪 CFG 块集合。

## 13. 优化 Pass：局部 Load 消除（Store-Load Forwarding）

### 13.1 实现概述

在单个基本块内前向扫描，将最近的标量 `STORE(base, idx)` 结果直接前推到同地址的 `LOAD`：

- 识别基址为变量、索引为空或常量的标量访问，记录地址 → 值的可用映射。
- 遇到匹配 `LOAD` 时替换为 `ASSIGN`，删除冗余内存访问。
- 遇到不同索引形式或 CALL 时清空映射，保持别名保守。

## 14. 优化 Pass：局部死 Store 消除（Dead Store Elimination）

### 14.1 实现概述

在基本块内自底向上扫描，跟踪“可能被后续读取的地址”集合：

- 访问形态：基址为变量，索引为空或常量的标量存取。
- 遇到 `STORE` 同地址且该地址不在活跃集合中，说明后续从未读取，直接删除（置 NOP）。
- 遇到同地址的存储后，当前写覆盖旧值，将该地址从活跃集合移除；遇到 `LOAD` 则将地址加入集合。
- 遇到 `CALL` 清空集合（保守视为可能读/写任意内存）。
...
## 17. CleanupPass（IR 清理）

### 17.1 实现概述

该 Pass 用于清理优化过程中产生的冗余 IR，属于“收尾型”优化：不改变程序语义，但能减少 IR 噪音，降低后端生成无意义指令的概率。

主要做三类局部（基本块级）变换：

- **删除 NOP**：很多优化会把指令标记为 NOP，最终需要统一清理。
- **删除自拷贝**：`ASSIGN x, x` 这类指令对语义无影响，会被直接移除。
- **折叠相邻 ASSIGN 链**：对形如 `t1 = t0; t2 = t1; t3 = t2` 的连续复制链，尽量将后续使用改为直接引用最初源（例如把 `t3` 的使用改为 `t0`），并移除中间复制。

该 Pass 通常放在默认 Quad 优化序列的末尾，配合 CopyProp/DCE 等 Pass，清理冗余指令。

## 18. ArrayBaseHoistPass（数组基址提升）

### 18.1 实现概述

该 Pass 针对基本块内的数组访问做一个非常局部的提取：当同一个数组对象在同一基本块内被多次以“带索引的 load/store”形式访问时，先把数组“基址”只计算/取一次，保存到临时寄存器中，随后将同块内后续的访问重写为使用该临时寄存器作为 base。

直观效果：减少重复的地址准备指令（在 MIPS 后端常表现为重复的 `la` / `addiu` 等），并缩短寄存器活跃区间。
...
- 仅对“真实数组对象”的 base 做提升：例如本地数组/全局数组。
- 对**数组形参**（表现为 `ALLOCA size == 1` 的“指针槽位”）必须跳过。

原因：数组形参在 IR 里通常是“指针的地址”（slot），对它执行 `LOAD base, Empty` 取到的是 slot 的地址而不是 slot 里存的指针值；若把这个结果当作数组基址继续参与后续 `LOAD/STORE`，会造成访存地址错误。
...
IR 的设计其实经过了多次迭代，详细可以看看`git log include/codegen/Instruction.hpp`，开始的时候IR设计过于高级，比如开始的`PARAM`的设计，
他担任了传参，分配空间等多种职责，我们是支持数组传参的，这意味着我需要有特殊判断，凡是涉及到数组的我都需要特殊判断，带来的大量
if else, 但其实完全可以把`PARAM`单纯设计成传参的职责，分配空间交给`ALLOCA`，这样不仅减少了特殊判断，并且有一种统一感,参数也可以加入
到`Mem2Reg`了。所谓传递地址,我就把地址当成一个值传递不就好了。
