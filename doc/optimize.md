...
- 在前端清掉常量分支/空循环，显著缩短后续 CSE/DCE/RA 的活跃区间和溢出压力。
- CFG 结构简化后，支配树/循环分析准确性提升。
...
- 减少 load/store 数量，缩小活跃区间，帮助寄存器分配与后端溢出下降。
- 与 SCCP、CopyProp、DCE 连用，可将被前推后无用的临时及时清理。
...
- 与前向 Load 消除/常量传播配合，显著减少冗余内存写，缩小 live 区间，降低寄存器溢出与后端内存流量。
